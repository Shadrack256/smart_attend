...
foreach ($students as $st) {
    ...
    foreach ($sessions as $s) {
        ...
        if ($status === 'present') {
            $cells .= '<td class="c mark-p">✓</td>';
            $present++;
        } elseif ($status === 'late') {
            $cells .= '<td class="c mark-l">L</td>';
            $present++;
        } else {
            $cells .= '<td class="c mark-a">✗</td>';
        }
    }
...
if (!$students) {
    $rowsHtml = '<tr><td colspan="' . (4 + $sessionCount) . '" class="c empty">No students enrolled in this course.</td></tr>';
} elseif (!$sessions) {
    $rowsHtml = '<tr><td colspan="4" class="c empty">No sessions found in the selected period.</td></tr>';
}
