...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Force role to student on public registration (lecturers/admins created by admin)
    $role = 'student';
    ...
    if ($name === '' || $email === '' || $password === '') {
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                ...
                VALUES (?, ?, ?, ?, ?)
            ");
            $stmt->execute([$reg ?: null, $name, $email, $hash, $role]);

            $_SESSION['flash'] = ['type' => 'success', 'msg' => 'Account created. You can now sign in.'];
            header("Location: login.php");
            exit;
        } catch (PDOException $e) {
            if ($e->getCode() === '23000') {
                $error = 'That email or registration number is already registered.';
            } else {
...
$pageTitle = 'Create account · ' . ($brand['system_name'] ?? 'SmartAttend');
require __DIR__ . '/../includes/head.php';
?>

<body class="bg-slate-50 text-slate-800 antialiased min-h-screen flex flex-col">

    <!-- Main area — centers the register card, holds the background -->
    <div class="relative flex-1 flex items-center justify-center p-6" style="<?= $bgStyle ?>">
        ...
        <div class="relative w-full max-w-md py-6">
            ...
            <div class="text-center mb-8">
                ...
                <h1 class="text-2xl font-bold <?= $bgFile ? 'text-white' : 'text-slate-900' ?>">
                    Create your account
                </h1>
                <p class="text-sm mt-1 <?= $bgFile ? 'text-white/70' : 'text-slate-500' ?>">
                    Join <?= htmlspecialchars($brand['system_name'] ?? 'SmartAttend') ?>
                </p>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-card p-8">
                ...
                <form method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input name="reg_number" placeholder="e.g. CSC/2024/001"
                               value="<?= htmlspecialchars($_POST['reg_number'] ?? '') ?>"
                               class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
                        <p class="text-xs text-slate-500 mt-1.5">Students only. Lecturers are added by an administrator.</p>
                    </div>
